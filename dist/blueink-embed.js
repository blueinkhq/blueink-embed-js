(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["BlueInkEmbed"] = factory();
	else
		root["BlueInkEmbed"] = factory();
})(window, function() {
...
// Module
exports.push([module.i, ".blueink-sig-iframe {\n    display: block;\n    width: 100%;\n    height: 100%;\n    min-height: 480px;\n    min-width: 320px;\n    border: none;\n}", ""]);
// Exports
...
/*!**********************!*\
  !*** ./src/embed.js ***!
  \**********************/
/*! exports provided: PUBLIC_API_KEY_PARAM, IFRAME_CLASSNAME, EVENT, BlueInkEmbedError, default */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "PUBLIC_API_KEY_PARAM", function() { return PUBLIC_API_KEY_PARAM; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "IFRAME_CLASSNAME", function() { return IFRAME_CLASSNAME; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "EVENT", function() { return EVENT; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "BlueInkEmbedError", function() { return BlueInkEmbedError; });
...
var PUBLIC_API_KEY_PARAM = 'publicAPIKey';
var IFRAME_CLASSNAME = 'blueink-sig-iframe';
/**
...
var MIN_PUBLIC_API_KEY_LENGTH = 40;
var PUBLIC_API_KEY_PREFIX = 'public_';
var ALLOWED_MOUNT_OPTIONS = ['class', 'debug', 'isTest', 'locale', 'redirectURL', 'replace'];
/**
 * Custom Error class thrown for some errors in BlueInkEmbed
 */

var BlueInkEmbedError = /*#__PURE__*/function (_Error) {
  _inherits(BlueInkEmbedError, _Error);

  var _super = _createSuper(BlueInkEmbedError);

  /**
   * Create a BlueInkEmbedError
   * @param {string} message - a message providing additional details about the error
   */
  function BlueInkEmbedError(message) {
    var _this;

    _classCallCheck(this, BlueInkEmbedError);

    _this = _super.call(this, message);
    _this.name = 'BlueInkEmbedError';
    return _this;
  }

  return BlueInkEmbedError;
...
var BlueInkEmbed = /*#__PURE__*/function (_EventEmitter) {
  _inherits(BlueInkEmbed, _EventEmitter);

  var _super2 = _createSuper(BlueInkEmbed);
   ...
   */
  function BlueInkEmbed(publicAPIKey) {
    var _this2;

    _classCallCheck(this, BlueInkEmbed);

    _this2 = _super2.call(this);

    _defineProperty(_assertThisInitialized(_this2), "_containerEl", null);

    _defineProperty(_assertThisInitialized(_this2), "_debugMode", false);

    _defineProperty(_assertThisInitialized(_this2), "_iFrameEl", null);

    _defineProperty(_assertThisInitialized(_this2), "_iFrameOrigin", null);

    _defineProperty(_assertThisInitialized(_this2), "_publicAPIKey", null);

    _defineProperty(_assertThisInitialized(_this2), "_receiveMessage", function (event) {
      if (event.origin !== _this2._iFrameOrigin) {
        return;
      }

      if (_this2._iFrameEl && event.source !== _this2._iFrameEl.contentWindow) {
        _this2._debug('Received message with correct origin but wrong source. Silently dropping.', event.data);

        return;
      }

      var eventType = event.data && event.data.eventType;

      if (!eventType) {
        _this2._debug('Received message with no eventType. Silently dropping.');

        return;
      }

      if (!Object.values(EVENT).includes(eventType)) {
        _this2._debug("Received message with unknown eventType \"".concat(eventType, "\". Silently dropping."));

        return;
      }

      _this2.emit(eventType, event.data);

      _this2.emit(EVENT.ANY, eventType, event.data);
    });
    ...
    if (publicAPIKey.length < MIN_PUBLIC_API_KEY_LENGTH) {
      throw new TypeError('publicAPIKey is too short. Please verify you are using a valid BlueInk public API key.');
    }

    if (!publicAPIKey.startsWith(PUBLIC_API_KEY_PREFIX)) {
      throw new TypeError('publicAPIKey is invalid. Please verify you are using a valid BlueInk public API key.');
    }

    _this2._publicAPIKey = publicAPIKey;
    return _this2;
  ...
  _createClass(BlueInkEmbed, [{
    key: "mount",
    value: function mount(embeddedSigningURL) {
      ...
      var cleanOptions; // allow container to be omitted

      if (lodash_isplainobject__WEBPACK_IMPORTED_MODULE_1___default()(container)) {
        cleanOptions = lodash_pick__WEBPACK_IMPORTED_MODULE_2___default()(container, ALLOWED_MOUNT_OPTIONS);
        container = 'body';
      } else {
        cleanOptions = lodash_pick__WEBPACK_IMPORTED_MODULE_2___default()(options, ALLOWED_MOUNT_OPTIONS);
      }

      var containerNodes = document.querySelectorAll(container);

      if (containerNodes.length > 1) {
        throw new BlueInkEmbedError("More than one element found matching container selector \"".concat(container, "\""));
      }

      if (containerNodes.length === 0) {
        throw new BlueInkEmbedError("Cannot find element matching container selector \"".concat(container, "\""));
      }
      ...
      var processedURL = this._buildFinalURL(embeddedSigningURL, cleanOptions);

      var className = cleanOptions.className || IFRAME_CLASSNAME;
      ...
      BlueInkEmbed._mounted = true; // FIXME - sanity checks on size of container element, after rendering
    }
    /**
    ...
    value: function removeMessageListener() {
     ...
     * @private
     */
    function _buildFinalURL(embeddedSigningURL, options) {
      var optionsWithKey = _objectSpread(_defineProperty({}, PUBLIC_API_KEY_PARAM, this._publicAPIKey), options);

      return "".concat(embeddedSigningURL, "?").concat(qs__WEBPACK_IMPORTED_MODULE_3___default.a.stringify(optionsWithKey));
    }
    /**
    ...
    key: "_createIframe",
    value: function _createIframe(url) {
      var className = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : IFRAME_CLASSNAME;
      var iFrame = document.createElement('iframe');

      var origin = this._extractOrigin(url);

      iFrame.className = className;
      iFrame.src = url;
      iFrame.allow = "camera ".concat(origin, "; geolocation ").concat(origin);
      return iFrame;
    }
    /**
    ...
    key: "_extractOrigin",
    value: function _extractOrigin(embeddedSigningURL) {
      var url = new URL(embeddedSigningURL);
      return url.origin;
    }
    /**
...
_defineProperty(BlueInkEmbed, "_mounted", false);

_defineProperty(BlueInkEmbed, "EVENT", EVENT);
